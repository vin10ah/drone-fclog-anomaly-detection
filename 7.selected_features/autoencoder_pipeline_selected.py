...
def run(mode, params, msg_field): 
    ...
    if mode == 'train':
        ...
        model = Autoencoder().to(device)
        model_path = f"./results/models/ae_bs{params['batch_size']}_lr{params['lr']:.0e}_ep{params['epochs']}.pth"
        model.load_state_dict(torch.load(model_path, map_location=device))

        threshold = compute_threshold(model, X_trn_tensor, quantile=params['quantile'])
        print(f"Threshold ({int(params['quantile'] * 100)}% quantile): {threshold:.6f}")

        nor_recon_error, nor_y_pred = predict(model, nor_tst_tensor, threshold)
        nor_y_true = np.zeros_like(nor_y_pred)

        ab_recon_error, ab_y_pred = predict(model, ab_tst_tensor, threshold)
        ab_y_true = np.ones_like(ab_y_pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--msg_feild', type=str, defalt=None, help='Choose messeage field')
